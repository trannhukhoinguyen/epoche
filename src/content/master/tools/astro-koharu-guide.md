...
在 `config/site.yaml` 中配置分类名到 URL slug 的映射：
...
// const MODEL_NAME = 'BAAI/bge-small-zh-v1.5';  // 针对优化
```
...
| `BAAI/bge-small-zh-v1.5`                  | ~95MB | 512  | 专用           |
...
- `vi.ts`：默认语言，包含所有翻译 key（约 170 个），是唯一的 source-of-truth
- `zh.ts`：默认语言，包含所有翻译 key（约 170 个），是唯一的 source-of-truth
- `en.ts`：英文翻译，只需提供需要覆盖的 key，未提供的自动回退到
